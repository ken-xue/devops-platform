...
export default {
  ...
  props: {
    socketURI: {
      type: String,
      default: 'ws://127.0.0.1:8088/ws/attach?f1944396bf43402aaa501b5856f67379'
    },
  },
  data() {
    return {
  ...
  methods: {
    ...
    init(data) {
      this.open = true
      this.initSocket(data,1)
      this.terminalUuid = data.nodeName
      this.editableTabs = [{
        title: '终端1',
        name: '1',
    ...
    initSocket(data,elementId) {
      this.socket = new WebSocket(`${process.env.VUE_APP_SOCKET_API}/ws/attach?clusterId=${data.clusterId}&namespace=${data.namespace}&name=${data.name}`);
      this.socketOnClose();
