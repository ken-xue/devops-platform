# install  =  kubeadm install kubernetes

[![GitHub Super-Linter](https://github.com/lework/install/workflows/Lint%20Code%20Base/badge.svg)](https://github.com/marketplace/actions/super-linter)
...
> 案例使用请见：[https://lework.github.io/2020/09/26/install](https://lework.github.io/2020/09/26/install)

### 下载脚本

```bash
# centos
wget https://cdn.jsdelivr.net/gh/lework/install@master/install-centos.sh

# debian
wget https://cdn.jsdelivr.net/gh/lework/install@master/install-debian.sh

# ubuntu
wget https://cdn.jsdelivr.net/gh/lework/install@master/install-ubuntu.sh
```
...
Example:
  ...
  [reset cluster]
  install-centos.sh reset \
  --user root \
  --password 123456

  [add node]
  install-centos.sh add \
  ...
  [other]
  install-centos.sh renew-cert --user root --password 123456
  install-centos.sh upgrade --version 1.20.6 --user root --password 123456
  install-centos.sh update
  install-centos.sh add --ingress traefik
  install-centos.sh add --monitor prometheus
  install-centos.sh add --log elasticsearch
  install-centos.sh add --storage rook
  install-centos.sh add --ui dashboard
  install-centos.sh add --addon nodelocaldns
```
...
```bash
bash -c "$(curl -sSL https://cdn.jsdelivr.net/gh/lework/install@master/install-centos.sh)"  \
  - init \
  --master 192.168.77.130,192.168.77.131,192.168.77.132 \
  --worker 192.168.77.133,192.168.77.134 \
  --user root \
...
```bash
# 增加单个master节点
bash install-centos.sh add --master 192.168.77.135

# 增加单个worker节点
bash install-centos.sh add --worker 192.168.77.134

# 同时增加
bash install-centos.sh add --master 192.168.77.135,192.168.77.136 --worker 192.168.77.137,192.168.77.138
```
...
```bash
# 删除单个master节点
bash install-centos.sh del --master 192.168.77.135

# 删除单个worker节点
bash install-centos.sh del --worker 192.168.77.134

# 同时删除
bash install-centos.sh del --master 192.168.77.135,192.168.77.136 --worker 192.168.77.137,192.168.77.138
```

### 重置集群

```bash
bash install-centos.sh reset \
  --user root \
  --password 123456 \
  --port 22 \
```
...
```bash
# 添加 nginx ingress
bash install-centos.sh add --ingress nginx

# 添加 prometheus
bash install-centos.sh add --monitor prometheus

# 添加 elasticsearch
bash install-centos.sh add --log elasticsearch

# 添加 rook
bash install-centos.sh add --storage rook

# 添加 nodelocaldns
bash install-centos.sh add --addon nodelocaldns

# 升级版本
bash install-centos.sh upgrade --version 1.20.6

# 重新颁发证书
bash install-centos.sh renew-cert

# debug模式
DEBUG=1 bash install-centos.sh

# 更新脚本
bash install-centos.sh update
...
KUBE_IMAGE_REPO="${KUBE_IMAGE_REPO:-registry.cn-hangzhou.aliyuncs.com/install}"
...
> 详细部署请见: [https://lework.github.io/2020/10/18/install-offline/](https://lework.github.io/2020/10/18/install-offline/)

1. 下载指定版本的离线包

    ```bash
    wget https://github.com/lework/install-offline/releases/download/1.20.6/1.20.6_centos7.tgz
    ```

    > 更多离线包信息，见 [install-offline](https://github.com/lework/install-offline) 仓库
