...
function script::init_node() {
  ...
  sed -i '/## install managed start/,/## install managed end/d' /etc/security/limits.conf /etc/systemd/system.conf /etc/bash.bashrc /etc/audit/rules.d/audit.rules
...
function update::self {
  ...
  command::exec "127.0.0.1" "
    wget --timeout=10 --waitretry=3 --tries=5 --retry-connrefused https://cdn.jsdelivr.net/gh/lework/install@master/install-debian.sh -O /tmp/install-debian.sh || exit 1
    /bin/mv -fv /tmp/install-debian.sh \"$0\"
    chmod +x \"$0\"
  "
